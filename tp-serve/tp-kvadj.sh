...
SELF="$REPO/tp-serve/tp-kvadj.sh"                  # path of this script on each box
# ----------------------------------------------------------------------------
...
env_common() {
    ...
    # [A/B] unified [raw|comp] KV page -> k_all is a view (1) vs ggml_concat copy (0)
    export DSV4_KV_ADJACENT="${DSV4_KV_ADJACENT:-1}"
    [ -n "${DSV4_OPPROF:-}" ]      && export DSV4_OPPROF DSV4_OPPROF_SKIP DSV4_OPPROF_DUMP_AFTER DSV4_OPPROF_BYOP
