...
env_common() {
    ...
    # [DSV4_MTP_FOLD] fold the NextN draft head into the trunk verify graph. Affects the
    # trunk KV memory layout (extra NextN SWA plane), so both ranks MUST agree -> forward.
    [ -n "${DSV4_MTP_FOLD:-}" ] && export DSV4_MTP_FOLD
...
case "${1:-}" in
    ...
    ALLRESTART)
        ...
        [ -n "${DSV4_MTP_FOLD:-}" ]         && FWD="$FWD DSV4_MTP_FOLD=$DSV4_MTP_FOLD"
