// A/B perf: plain indexed bf16 gather vs TMA-Gather4, identical kernel otherwise, PREFILL shape.
// Measures per-query throughput (q/s) over realistic ubatch waves. Both gather from bf16 k_all.
#include "ref.h"
#include <mma.h>
...
__global__ void __launch_bounds__(256) k_plain(const float* Qall,const bf16_t* K,const int* kv_idx,
        int top_k,int kv_row_elems,int n_comp_view,int n_kv,float scale,float* Oall){
    COMMON_HEAD __syncthreads();
...
__global__ void __launch_bounds__(256) k_tma(const __grid_constant__ CUtensorMap tmK,const float* Qall,const int* kv_idx,
        int top_k,int kv_row_elems,int n_comp_view,int n_kv,float scale,float* Oall){
    COMMON_HEAD
    __shared__ alignas(128) __nv_bfloat16 sStage[4*TMA_W]; __shared__ alignas(8) uint64_t bar; __shared__ int sRow[KB];
...
int main(){
    ...
    int M=getenv("M")?atoi(getenv("M")):2000;
    int n_comp_view=getenv("NCV")?atoi(getenv("NCV")):3000;
    int top_k=512; int n_kv=((n_comp_view+255)/256)*256;
    ...
    std::mt19937 rng(123);
    for(int it=0;it<M;it++){ for(size_t i=0;i<(size_t)HQ*D;i++)Qall[(size_t)it*HQ*D+i]=p.Q[i]*(1.f+0.001f*it);
    ...
    cudaFuncSetAttribute(k_plain,cudaFuncAttributeMaxDynamicSharedMemorySize,(int)smem);
    cudaFuncSetAttribute(k_tma,cudaFuncAttributeMaxDynamicSharedMemorySize,(int)smem);
    const int IT=50;
    // warmup + time plain
    for(int w=0;w<5;w++) k_plain<<<M,256,smem>>>(dQ,dK,dKv,top_k,kv_row_elems,n_comp_view,n_kv,p.scale,dO);
    cudaDeviceSynchronize();
    auto t0=std::chrono::high_resolution_clock::now();
    for(int i=0;i<IT;i++) k_plain<<<M,256,smem>>>(dQ,dK,dKv,top_k,kv_row_elems,n_comp_view,n_kv,p.scale,dO);
    cudaDeviceSynchronize();
    auto t1=std::chrono::high_resolution_clock::now();
    double pl_ms=std::chrono::duration<double,std::milli>(t1-t0).count()/IT;
    for(int w=0;w<5;w++) k_tma<<<M,256,smem>>>(tm,dQ,dKv,top_k,kv_row_elems,n_comp_view,n_kv,p.scale,dO);
    cudaDeviceSynchronize();
    auto t2=std::chrono::high_resolution_clock::now();
    for(int i=0;i<IT;i++) k_tma<<<M,256,smem>>>(tm,dQ,dKv,top_k,kv_row_elems,n_comp_view,n_kv,p.scale,dO);
    cudaDeviceSynchronize();
    auto t3=std::chrono::high_resolution_clock::now();
    double tm_ms=std::chrono::duration<double,std::milli>(t3-t2).count()/IT;
    printf("GPU %s sm_%d%d  M=%d n_comp=%d top_k=%d\n",pr.name,pr.major,pr.minor,M,n_comp_view,top_k);
    printf("PLAIN gather: %.3f ms/launch  -> %.0f q/s\n",pl_ms, M/(pl_ms/1000.0));
    printf("TMA   gather: %.3f ms/launch  -> %.0f q/s\n",tm_ms, M/(tm_ms/1000.0));
    printf("SPEEDUP (TMA vs PLAIN) = %.2fx\n", pl_ms/tm_ms);
    return 0;
}
