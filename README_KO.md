...
llama.cpp에 vLLM의 앞서 있는 훌륭한 커널 기술을 이식하고자 3일이 넘도록 Claude Code로 작업하였으나, 임무를 완수하는 데 실패하였습니다. 수도 없이 반복되는 고통스러운 개발을 — 마치 스테이지의 보스들을 하나씩 쓰러뜨려 가는 듯한 고통으로 — 극복해 왔으나, 이제 한계에 봉착하였습니다.

Aiden의 vLLM은 초당 **1,600 토큰 prefill**과 **초당 45 토큰 디코딩**을 달성하였습니다. 그러나 우리는 **동일한 하드웨어**를 갖고도 초당 **330 토큰 prefill**밖에 달성할 수 없었습니다. 디코딩은 초당 **13~19.5 토큰**을 넘지 못했습니다.

이에 llama.cpp 아키텍처에 대한 개선을 포기하고, 폐기 처분하는 눈물의 결정을 내려야만 했습니다.

Claude Code에서 **Opus 4.8**과 같은 세계 최정상 인공지능으로도 이 작업을 극복할 수 없었습니다. **Fable 5**가 설령 대중에게 공개되더라도, 저는 마찬가지로 인공지능이 이를 해결할 수 없으리라 생각합니다. 저는 인공지능을 사용하여 llama.cpp에 vLLM의 최고급 최신 기술을 이식하는 작업을 더 이상 하지 않을 것이며, **이 레포지터리는 아카이빙됩니다.**

그동안 즐거웠습니다. 슬프지만 프로젝트 폐기를 공표하오니 양해 바랍니다. *(모두 vLLM을 사용하십시오. 그것이 여러분의 정신 건강을 해치지 않을 것입니다. 개인적으로는, 매우 슬픈 하루가 될 것 같습니다.)*
