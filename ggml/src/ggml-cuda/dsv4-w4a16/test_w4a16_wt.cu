...
__global__ void gemm_wt(const __nv_bfloat16* __restrict__ A, const uint8_t* __restrict__ Bc,
...
int main(){
    ...
    gemm_wt<<<grid,block>>>(dA,dBc,(uint8_t)SBYTE,dD);
    ...
    for(int w=0;w<5;w++) gemm_wt<<<grid,block>>>(dA,dBc,(uint8_t)SBYTE,dD);
    cudaEventRecord(t0); for(int it=0;it<iters;it++) gemm_wt<<<grid,block>>>(dA,dBc,(uint8_t)SBYTE,dD);
    cudaEventRecord(t1); cudaEventSynchronize(t1); float ms=0; cudaEventElapsedTime(&ms,t0,t1);
    double flop=2.0*M*N*K*iters;
    printf("wt BM%d BN%d BK%d WM%d WN%d (RM%d RN%d w%d) %s | parity %d/%d | %.3f ms | %.1f GFLOP/s (peak 75.9k)\n",
           BM,BN,BK,WM,WN,RM,RN,NWARPS, e==cudaSuccess?"ok":"ERR", checked-fails, checked, ms/iters, flop/(ms/1e3)/1e9);
    return fails?1:0;
}
