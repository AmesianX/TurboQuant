...
// ---------------------------------------------------------------------------
// SPMD cross-node tensor parallelism (feat/tp-2node-dsv4).
// Each node runs one process with one local GPU and joins a global NCCL world of
// GGML_TP_NRANKS ranks via ncclCommInitRank. The ncclUniqueId is generated by
// rank 0 and broadcast to the others over a tiny TCP bootstrap (no MPI). This is
// the canonical SPMD path used by vLLM/Megatron, adapted to ggml's comm context.
// Transport verified by tools/tp-bench (RDMA over RoCE, ~26us/AllReduce @ 16KB).
// ---------------------------------------------------------------------------
#if defined(GGML_USE_NCCL) && defined(__linux__)
#include <unistd.h>
#include <arpa/inet.h>
#include <sys/socket.h>
#include <netinet/in.h>

// rank 0 listens and sends `id` to every other rank; ranks >0 connect to master
// and receive it. Returns true on success. Blocks until the exchange completes.
static bool tp_bootstrap_uniqueid(int rank, int nranks, const char * master_addr,
                                  int port, ncclUniqueId * id) {
    const size_t N = sizeof(ncclUniqueId);
    if (rank == 0) {
        int s = socket(AF_INET, SOCK_STREAM, 0);
        if (s < 0) { GGML_LOG_ERROR("TP bootstrap: socket failed\n"); return false; }
        int one = 1; setsockopt(s, SOL_SOCKET, SO_REUSEADDR, &one, sizeof(one));
        sockaddr_in a{}; a.sin_family = AF_INET; a.sin_port = htons(port); a.sin_addr.s_addr = INADDR_ANY;
        if (bind(s, (sockaddr*)&a, sizeof(a)) < 0) { GGML_LOG_ERROR("TP bootstrap: bind %d failed\n", port); close(s); return false; }
        listen(s, nranks);
        for (int peer = 1; peer < nranks; ++peer) {
            int c = accept(s, nullptr, nullptr);
            if (c < 0) { GGML_LOG_ERROR("TP bootstrap: accept failed\n"); close(s); return false; }
            size_t off = 0; const char * p = (const char *) id;
            while (off < N) { ssize_t w = write(c, p + off, N - off); if (w <= 0) { close(c); close(s); return false; } off += (size_t) w; }
            close(c);
        }
        close(s);
    } else {
        ...
        for (int attempt = 0; attempt < 18000; ++attempt) {
            ...
            close(s); s = -1; usleep(200000); // 0.2s; wait for rank 0 to come up
        }
        if (s < 0) { GGML_LOG_ERROR("TP bootstrap: connect to %s:%d failed\n", master_addr, port); return false; }
        size_t off = 0; char * p = (char *) id;
        while (off < N) { ssize_t r = read(s, p + off, N - off); if (r <= 0) { close(s); return false; } off += (size_t) r; }
        close(s);
    }
    return true;
}

// SPMD init: one local GPU, global NCCL world of GGML_TP_NRANKS. Returns true if
// SPMD mode was requested (NRANKS>1) and brought up; false means "not SPMD, use
// the normal single-process path".
static bool ggml_backend_cuda_comm_init_nccl_spmd(ggml_backend_cuda_comm_context * ret) {
    const char * e_nranks = getenv("GGML_TP_NRANKS");
    const int nranks = e_nranks ? atoi(e_nranks) : 1;
    if (nranks <= 1) {
        return false; // not SPMD
    }
    const int rank = getenv("GGML_TP_RANK") ? atoi(getenv("GGML_TP_RANK")) : 0;
    ...
    if (!tp_bootstrap_uniqueid(rank, nranks, master, port, &id)) {
        GGML_LOG_ERROR("TP SPMD: uniqueId bootstrap failed (rank %d/%d, master %s:%d)\n", rank, nranks, master, port);
        ggml_backend_cuda_comm_init_none(ret);
        return true; // SPMD requested but failed; do not fall through to ncclCommInitAll
    }

    ggml_cuda_set_device(ret->dev_ids[0]);
    ret->comms.resize(1);
    ncclResult_t rc = ncclCommInitRank(&ret->comms[0], nranks, id, rank);
    if (rc != ncclSuccess) {
        GGML_LOG_ERROR("TP SPMD: ncclCommInitRank failed: %s\n", ncclGetErrorString(rc));
        ret->comms.clear();
        ggml_backend_cuda_comm_init_none(ret);
        return true;
    }
    GGML_LOG_INFO("TP SPMD: NCCL world up (rank %d/%d, dev %d, master %s:%d)\n",
                  rank, nranks, ret->dev_ids[0], master, port);
    ret->try_allreduce = ggml_backend_cuda_comm_try_allreduce_nccl;
    return true;
}
#else
static bool ggml_backend_cuda_comm_init_nccl_spmd(ggml_backend_cuda_comm_context *) { return false; }
#endif // GGML_USE_NCCL && __linux__

static void ggml_backend_cuda_comm_init_nccl(ggml_backend_cuda_comm_context * ret) {
#ifdef GGML_USE_NCCL
    // Cross-node SPMD (GGML_TP_NRANKS>1) takes precedence over single-process init.
    if (ggml_backend_cuda_comm_init_nccl_spmd(ret)) {
        return;
    }
...
static bool ggml_cuda_graph_check_compability(ggml_cgraph * cgraph) {
    ...
    // [DSV4] global kill-switch: GGML_CUDA_NO_GRAPHS=1 forces fully-eager execution for the
    // WHOLE graph (every sub-graph), bypassing all per-op gating. Used to isolate graph-capture
    // crashes from real kernel OOBs and to get a stable crash-free baseline measurement.
    static const bool no_graphs_env = getenv("GGML_CUDA_NO_GRAPHS") != nullptr;
    if (no_graphs_env) {
        return false;
    }
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        // [DSV4_MOE_FUSED] CUDA-graph capture is SAFE for the flashinfer CUTLASS fused MoE,
        // so graphs are ON by default (no graph-off workaround).
        //
        // The earlier long-context crash was NOT a graph-capture defect in runMoe — it was a
        // freed-weight use-after-null in the GROUPED small-prefill fallback (under DSV4_MOE_FUSED
        // the fused path frees the grouped dq_gate/dq_up, and a 16<M<256 chunk then read them);
        // that is fixed in dsv4-moe-grouped.cu (route the fused-live small-prefill band through
        // the fused-aware HP kernels). The fused CUTLASS runner itself is compute-sanitizer-clean
        // over 60 imbalanced M<=1024 chunks incl. the high-water grow path (standalone proof).
        //
        // Why capture is safe: runMoe's internal layout IS M-keyed (configureWsPtrs(M),
        // computeNumTokensPerBlock(M) pick offsets + a grid dim3(E, ceilDiv(M,blk))). But:
        //   (1) ggml's graph cache keys by (nodes[0], n_nodes) AND re-captures whenever a node's
        //       src ne changes (ggml_cuda_graph_update_required), so EACH distinct M gets its OWN
        //       captured graph — a graph is never replayed at a different M than it was captured at.
        //   (2) Our op pins the shared workspace + all on-device scratch to tok_cap = max ubatch
        //       ONCE (dsv4-moe-fused-run.cu), so a captured graph's workspace base + sub-buffer
        //       offsets stay valid across replays (no realloc, no retire mid-capture).
        // Together => for any fixed M the captured pointers/grids are constant across replays => safe.
        // A/B / safety hatch: DSV4_MOE_FUSED_GRAPH_OFF=1 forces graphs OFF for this op only.
        ...
        if (node->op == GGML_OP_DSV4_MOE_GROUPED) {
            ...
            // [FUSED-LIVE small-prefill band] Under DSV4_MOE_FUSED a 16<M<min_m chunk (the rare
            // partial last prefill chunk) is routed through the grouped fused-aware HP path, which
            // may grow its FP32 act buffer (cudaMalloc) on first hit. A mid-capture malloc =
            // "internal operation failed". The HP path pins that buffer to the worst-case band size
            // once, but the FIRST band hit still allocates — so keep graphs OFF for the band (it is
            // an infrequent partial chunk, no steady-state perf cost). hp_max < M < min_m identifies it.
            static const int fused_min_m = []{
        ...
        case GGML_OP_SET_ROWS:
            {
                return (op->type == GGML_TYPE_F32 || op->type == GGML_TYPE_F16 || op->type == GGML_TYPE_BF16 ||
                       ...
                       op->type == GGML_TYPE_AMX3_1 || op->type == GGML_TYPE_AMXV3_1 ||
                       op->type == GGML_TYPE_TBQV3_1) &&
