...
// LOSSY approx-dp4a path for F8_E4M3_B128 — gated, default OFF. Set GGML_CUDA_F8_APPROX_DP4A to a
// non-"0" value to enable. The default (shared-LUT) path stays bit-exact with the original vec_dot.
...
static __global__ void mul_mat_vec_q(
    ...
    // F8 LUT staging. shared-LUT (exact) is the DEFAULT for F8 decode; it is bit-identical to the
    // global/__ldg path, just faster memory. approx-dp4a is LOSSY and only active when the env gate
    // selected the f8_approx_dp4a template instantiation.
...
#pragma unroll
            for (int i = 0; i < rows_per_cuda_block; ++i) {
                ...
                } else if constexpr (stage_iq2xs_grid) {
...
static void mul_mat_vec_q_switch_fusion(
    ...
    if constexpr (c_ncols_dst == 1) {
        if (has_fusion) {
            ...
            ggml_cuda_kernel_launch(mul_mat_vec_q<type, c_ncols_dst, true, small_k, f8_approx_dp4a>, launch_params,
    ...
    const ggml_cuda_kernel_launch_params launch_params = ggml_cuda_kernel_launch_params(block_nums, block_dims, nbytes_shared, stream);
    ggml_cuda_kernel_launch(mul_mat_vec_q<type, c_ncols_dst, false, small_k, f8_approx_dp4a>, launch_params,
        vx, vy, ids, fusion, dst, ncols_x, nchannels_y, stride_row_x, stride_col_y, stride_col_dst,
        channel_ratio, stride_channel_x, stride_channel_y, stride_channel_dst,
        sample_ratio, stride_sample_x, stride_sample_y, stride_sample_dst, ids_stride);
}

// Runtime selector: turns the runtime f8_approx_dp4a flag into the compile-time template param.
// For non-F8 types this collapses to the plain (false) instantiation.
...
static void mul_mat_vec_q_switch_ncols_dst(
    ...
    // LOSSY approx-dp4a for F8 dense GEMV, env-gated (default OFF). Non-F8 types ignore this.
