...
// contiguous kernels
// note: templated on element type — DeepSeek-V4 concatenates f16 KV/state tensors
// (CPU fallback for those caused excessive graph splits), so T = float or half.
...
template <typename T>
static void concat_cuda_t(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
