...
| 🔀 Continuous batching | multi-slot (`-np 2`) → ~1.4× aggregate (~22.8 t/s @ 1M) |
...
- **MTP speculative decoding made viable on the FP4 stack — 16.6 t/s, beating plain.** On FP4 + MTP, speculation first ran *slower* than plain decode because the full model graph was being rebuilt every speculative round; reusing that graph across rounds removes the cost and lifts it to **16.6 t/s, past the no-speculation baseline**. It stays opt-in (perplexity-sensitive) and was verified lossless (greedy `France→Paris` / `수도→서울`, stable across both boxes).
- **2-node TP + multi-slot + MTP, composed.** SPMD tensor parallelism across two Sparks over RoCE (mirrored output, NCCL all-reduce), the multi-slot decode-batching path (`DSV4_MULTISLOT`, concurrent requests → **~1.4× aggregate** — measured 2-box Q4 @ 1M: ~16 t/s single → ~22.8 t/s with 2 concurrent), and MTP all run together. The enabling fix is a **graph-scoped metadata rebuild** in the meta-backend so the multi-slot + MTP verify graph no longer corrupts across TP splits.
