...
struct ggml_backend_meta_split_state llama_meta_device_get_split_state(const struct ggml_tensor * tensor, void * userdata) {
    ...
    auto get_tensor_config = [&]() -> tensor_config {
        // [tp-2node-dsv4] (direction B) MIRROR the whole NextN/MTP draft layer (the appended last
        ...
        if (hparams.nextn_predict_layers > 0 && tensor_name.compare(0, 4, "blk.") == 0) {
            ...
            if (dot != std::string::npos) {
                ...
                if (blk >= hparams.n_layer - hparams.nextn_predict_layers) {
                    return {GGML_BACKEND_SPLIT_AXIS_MIRRORED, tensor, blk, 0};
                }
            }
        }
        // standard attention
...
llama_memory_i * llama_model::create_memory(const llama_memory_params & params, const llama_cparams & cparams) const {
    ...
    switch (arch) {
        ...
        case LLM_ARCH_LLADA_MOE:
        case LLM_ARCH_RND1:
        case LLM_ARCH_DFLASH: // block-diffusion drafter: full bidirectional attn, no kv-cache
            {
                res = nullptr;
            } break;
        case LLM_ARCH_DEEPSEEK32:
