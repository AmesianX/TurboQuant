...
\usepackage{multirow}
\usepackage{url}
% Fallback CJK font for occasional Korean examples in the English paper
\usepackage{xeCJK}
\setCJKmainfont{Noto Serif CJK KR}[Scale=0.92]
...
More importantly, the Rayleigh distribution admits a \textbf{closed-form 8-level Lloyd-Max codebook} in $\sigma$-normalized coordinates that is independent of any particular model or dataset. Numerical solution of the standard Lloyd-Max iteration on the Rayleigh pdf yields the boundary vector $\mathbf{b}$ and centroid vector $\mathbf{c}$:
\begin{equation}
\small
\begin{aligned}
