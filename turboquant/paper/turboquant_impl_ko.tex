...
\subsection{Double WHT Per-Head (\_3 타입)}
\label{sec:double-wht}

$d=64$ head dimension 모델에서는 single 64-point WHT의 CLT 수렴이 불충분하다 (excess kurtosis $\sim 3/\sqrt{64} \approx 0.375$). Cross-head WHT ($H_{512} = H_8 \otimes H_{64}$, 8개 head 결합)를 시도했으나 \textbf{실패}: Q는 64-WHT 도메인에 있는데 K는 512-WHT 도메인에 있어 어텐션 스코어가 완전히 붕괴했다.

\paragraph{해결: Double WHT.} 독립적인 랜덤 부호 패턴 $S_1, S_2$로 WHT를 2회 적용한다:
\begin{equation}
\mathbf{y} = H_{64}\, S_2\, H_{64}\, S_1\, \mathbf{x}
\end{equation}
K(인코딩)와 Q(스코어링) 모두에 동일한 double WHT를 적용하여 도메인 일치를 보장한다. Parseval 정리에 의해 $\langle \text{dWHT}(\mathbf{q}),\, \text{dWHT}(\mathbf{k}) \rangle = D^2 \langle \mathbf{q}, \mathbf{k} \rangle$이므로, 역변환 없이 double-WHT 도메인에서 직접 내적을 계산할 수 있다.

Double WHT는 excess kurtosis $\sim 3/D = 0.047$ (near-Gaussian)을 달성하며, 이는 single WHT $D=512$의 $\sim 3/\sqrt{512} = 0.133$보다 \emph{우수}하다.

\paragraph{$d=64$에서 QJL 재활성화.} Double WHT의 개선된 CLT 수렴으로 QJL 랜덤 프로젝션 노이즈가 허용 범위 내에 들어온다. K는 항상 TBQP3\_3 (2비트 Lloyd-Max + 1비트 QJL + double WHT)으로 자동 매핑된다. QJL의 추가 1비트는 멀티턴에서 품질을 극적으로 개선한다: TBQ3\_3(QJL 없음)은 3--4턴에서 반복 루프, TBQP3\_3(QJL 포함)은 7턴 이상 정확한 답변을 유지한다.
