...
동작을 위해 두 가지 핵심 수정이 필요했다:

\paragraph{Per-head 정규화.} 초기 구현은 512개 WHT 계수 전체에 대한 global L2 norm을 사용했다. 한 head의 에너지가 지배적이면 나머지 head는 부적절한 스케일로 양자화되어 정보를 잃는다. $d=512$ per-block norm과 동일한 원리로 per-head RMS 정규화로 전환했다.

\paragraph{$d=64$에서 K의 QJL 제거.} TBQP 타입은 $(b{-}1)$비트 Lloyd-Max + 1비트 QJL을 사용한다. $d=64$에서 QJL의 랜덤 프로젝션은 어텐션 스코어에 올바른 토큰과 차점자 사이의 signal gap을 \emph{초과}하는 노이즈를 추가한다. 순수 3비트 Lloyd-Max(TBQ3, 8 centroid)는 TBQP3(2비트+QJL) 대비 $2.3\times$ 높은 스코어 SQNR을 달성한다. $d=64$에서 K를 자동으로 TBQP$\to$TBQ로 전환하여 모든 비트를 centroid 정밀도에 집중한다.
