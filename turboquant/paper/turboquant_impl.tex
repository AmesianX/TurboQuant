...
\subsection{Cross-Head WHT (\_3 Types)}

For models with $d=64$ head dimension, the 64-point WHT provides insufficient decorrelation. We implement a cross-head variant using the Kronecker structure $H_{512} = H_8 \otimes H_{64}$: eight heads share a single quantized KV block, with the $H_8$ signs applied at scoring time:
\begin{equation}
\text{score}_h = \frac{1}{8}\sum_{g=0}^{7} H_8[h,g] \cdot \langle \mathbf{q}_h, \text{dequant}(\mathbf{k}_g) \rangle
\end{equation}

Two critical fixes were required for cross-head to function:

\paragraph{Per-head normalization.} The initial implementation used a global L2 norm across all 512 WHT coefficients. When one head dominates in energy, the remaining heads are quantized with an inappropriate scale, losing information. We switch to per-head RMS normalization (same principle as per-block norm for $d=512$).

\paragraph{QJL removal for K at $d=64$.} TBQP types use $(b{-}1)$-bit Lloyd-Max plus 1-bit QJL. At $d=64$, the QJL random projection adds noise to the attention score that \emph{exceeds} the signal gap between correct and runner-up tokens. Pure 3-bit Lloyd-Max (TBQ3, 8 centroids) achieves $2.3\times$ better score SQNR than TBQP3 (2-bit + QJL). We automatically remap K from TBQP$\to$TBQ at $d=64$, dedicating all bits to centroid precision.
