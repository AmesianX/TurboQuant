...
\item \textbf{Multi-Variant Engineering} (\S\ref{sec:engineering}): We implement 20 TBQ/TBQP type variants across block sizes 64--576, including cross-head WHT ($H_8 \otimes H_{64}$), asymmetric K/V dimensions (GLM 576/512), per-block vs.\ global normalization, and QJL residual correction---all within the template-instantiated flash attention framework.
\end{enumerate}
...
\subsection{Double WHT Per-Head (\_3 Types)}
\label{sec:double-wht}

For models with $d=64$ head dimension, a single 64-point WHT provides insufficient CLT convergence (excess kurtosis $\sim 3/\sqrt{64} \approx 0.375$), resulting in a non-Gaussian distribution that degrades Lloyd-Max quantization quality. Cross-head WHT ($H_{512} = H_8 \otimes H_{64}$, combining 8 heads) was attempted but \textbf{failed}: Q remains in the 64-WHT domain while K is in the 512-WHT domain, creating a domain mismatch that corrupts attention scores.

\paragraph{Solution: Double WHT.} We apply two rounds of WHT with independent random sign patterns $S_1, S_2$:
\begin{equation}
\mathbf{y} = H_{64}\, S_2\, H_{64}\, S_1\, \mathbf{x}
\end{equation}
The same double WHT is applied to both K (during encoding) and Q (during scoring), ensuring domain agreement. By Parseval's theorem, $\langle \text{dWHT}(\mathbf{q}),\, \text{dWHT}(\mathbf{k}) \rangle = D^2 \langle \mathbf{q}, \mathbf{k} \rangle$, so the dot product can be computed directly in the double-WHT domain without inverse transform.

The double WHT achieves excess kurtosis $\sim 3/D = 0.047$ (near-Gaussian), \emph{better} than single WHT at $D=512$ ($\sim 3/\sqrt{512} = 0.133$). This enables optimal Lloyd-Max performance at $d=64$ without cross-head mixing.

\paragraph{QJL re-enabled at $d=64$.} With the improved CLT convergence from double WHT, the QJL random projection noise is now within acceptable bounds. K is always mapped to TBQP3\_3 (2-bit Lloyd-Max + 1-bit QJL + double WHT). The QJL extra bit provides critical quality improvement for multi-turn conversations: TBQ3\_3 (no QJL) degrades at 3--4 turns, while TBQP3\_3 (with QJL) maintains quality through 7+ turns with factually correct responses.
...
\begin{tabular}{@{}lccc@{}}
\toprule
Config & Avg (/35) & Peak & 10-run scores \\
\midrule
FP16 & 20.1 & 21 & 19--21 \\
\textbf{TBQP3/TBQ3} & \textbf{19.1} & \textbf{23} & 16--23 \\
\bottomrule
\end{tabular}
\caption{Math benchmark results. TBQ peak (23) exceeds FP16 best (21), suggesting quantization noise occasionally breaks out of local optima.}
\label{tab:math}
\end{table}
