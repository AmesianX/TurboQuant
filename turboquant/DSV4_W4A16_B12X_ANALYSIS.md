...
## 2. DECODE — FP4 GEMV micro-kernel (`micro.py MoEMicroKernelBackend`) — SEPARATE, NOT STARTED
- Chosen FIRST for m∈{1,2,4,8}, bf16, modelopt layout (`_W4A16SmallMDirectKernel`, kernel.py:426). THE prod decode path.
- **No mma.sync.** `fp4_dot4_sum`/`fp4_dot8_sum` (micro.py) = CUDA-core FP4×bf16 dot products + `warp_reduce`.
- Activations hoisted into registers ("no smem read"), weights streamed from global + dequant on the fly.
- Fully fused FC1→swiglu→FC2 in ONE launch, single device barrier. Bandwidth-bound: decode latency ≈ time
  to stream each expert's weights once. m==1 gets `single_token`/`share_input_across_experts` specialization.
- (TC-decode = optional packed-GEMM epilogue-fusion variant, default OFF; not the fast path.)
