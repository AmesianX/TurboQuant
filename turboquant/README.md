...
9. **Double WHT for head_dim=64 K (cross-head→per-head)**: Cross-head 512-point WHT failed at D=64 — Q-K domain mismatch (K in 512-WHT, Q in 64-WHT) caused attention score corruption. Solution: **double WHT per-head** (S1→WHT64→S2→WHT64) achieves kurtosis ~0.047 (near-Gaussian), better CLT convergence than single WHT D=512 (~0.133), while keeping Q and K in the same domain. No inverse WHT needed (Parseval). K auto-mapped to TBQP3_3 (double WHT + QJL) for all 3-bit head_dim=64 models.
10. **head_dim=64 K: QJL always enabled**: Double WHT's improved CLT convergence reduces QJL projection noise enough for reliable attention scoring at D=64. K always uses TBQP3_3 (2-bit Lloyd-Max + 1-bit QJL + double WHT) — QJL's extra bit provides critical quality boost for multi-turn (7+ turns). V keeps original type (tbq3_2, single WHT).
...
- `head_dim=64`: K=**tbqp3_3** (double WHT + QJL), V=tbq3_2 → **4.6x 압축**
...
- TurboQuant 논문은 head_dim=128만 검증 — head_dim=64는 single WHT의 CLT 수렴 부족
- **Cross-head WHT (8 head × 64 = 512-point) 시도 → 실패**: Q-K 도메인 불일치 (K는 512-WHT, Q는 64-WHT) → attention score 완전 붕괴
- **해결: Double WHT per-head** (S1→WHT64→S2→WHT64): kurtosis ~0.047 (near-Gaussian), Q와 K가 같은 도메인, 역양자화 불필요
- **QJL 1-bit 추가** (TBQP3_3): double WHT의 개선된 CLT 수렴으로 QJL 프로젝션 노이즈가 허용 범위 내 → 7턴+ 멀티턴 성공
...
1. **q8_0 fallback** (v1.2.0): K를 q8_0(8-bit)으로 — 파울리 통과, 2.7x 압축. WHT 3-bit의 CLT 한계를 우회.
2. **Cross-head WHT** (v1.5.2): 8 head × 64 = 512-point WHT로 CLT 수렴 확보 시도 → **실패**. Q(64-WHT)와 K(512-WHT) 도메인 불일치로 attention score 붕괴. H_8 sign trick과 IWHT 복원 모두 실용적 품질 달성 못함.
3. **Double WHT per-head** (v1.6.0): S1→WHT64→S2→WHT64로 2회 변환. kurtosis 0.375 → 0.047 (single WHT D=512의 0.133보다 우수). Q와 K 모두 같은 double-WHT 도메인 → 역양자화 불필요, Parseval 성립.
4. **Double WHT + QJL** (v1.6.0): TBQP3_3 (2-bit Lloyd-Max + 1-bit QJL + double WHT). QJL의 1-bit 추가가 멀티턴 안정성을 극적으로 개선 — 7턴+ 멀티턴, 전문 정답. K=tbq3(QJL 없음)은 3-4턴에서 반복 루프, K=tbqp3(QJL 있음)은 7턴 전문 정답.

**교훈:**
1. KV 캐시 양자화는 반드시 PPL뿐 아니라 실제 생성 품질 테스트(파울리 테스트)로 검증해야 합니다.
2. Cross-head WHT는 K 압축 효율을 올리지만 Q-K 도메인 불일치라는 근본 문제가 있습니다. **Q와 K는 반드시 같은 도메인에 있어야 합니다.**
3. Double WHT는 cross-head 없이 per-head에서 충분한 CLT 수렴을 달성하는 열쇠입니다.

> **참고:** _3 타입(tbq3_3, tbqp3_3 등)은 v1.6.0부터 "cross-head"가 아닌 "**double WHT per-head**"를 의미합니다. 각 head를 독립적으로 처리하되 WHT를 2회 적용하여 CLT 수렴을 확보합니다.
